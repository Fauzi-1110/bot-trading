...
async function bot_on() {
    ...
    allBot.forEach(([id, data]) => {
        ...
        if (data.role_play?.rest === true){  on_persen = 0; check_bot_rest(id)}
    ...
    for(let id of botOn){
        ...
        kerjaBot(id) //lempar ke bot kerja
        updateBotName(id) //update nama bot
        if(bot_data.role_play.health <= 10 || bot_data.role_play.rest === true) check_bot_rest(id) //cek status, apakah rest atau tidak
        if(bot_data.role_play.health <= 20 && Math.random() > 0.5) buy_role_play(id, "health") //kalau health dikit, isi
        if(bot_data.role_play.energy <= 10) buy_role_play(id, "energy") //kalau energi dikit, beli energy
        if(bot_data.uang > 5000000000000) buat_bisnis(id) //kalau banyak uang buat bisnis
        
        transaksi(id).normalBuy()
    }
    
    console.log(c.blueBright(`diakhiri bot id: ${botOn[botOn.length-1]} waktu: ${moment().tz("Asia/Jakarta").format("HH:mm:ss")}`))
}

function transaksi(id) {
    return {
        normalBuy: async function(){
            ...
            if (haved) {
                ...
                if (user_party.type === "borongan" && item_sell_data.nama_global === user_party.beli_barang) { this.partyBorongan(id); return }
                ...
                if (bot_data.role === "super" || bot_data.role === "smart") return this.smartBuy(id)  
                if (user_party && user_party.type === "smart") return this.smartBuy(id)
                if (user_party && user_party.type === "borongan") return this.partyBorongan(id)
            ...
            data_temp.role_play = {}
            data_temp.role_play.energy = "add -1"
            dbSystem.updateUser(id, data_temp)
        },
        smartBuy: async function(){
            const [global_object,bot_data, data_temp] = [dbSystem.getGlobal(), dbSystem.getUser(id), {}]
...
const party_fc = {
    add_party_borongan: async function(id) {
        const random_list_borong = ["hargaPerak",
            "hargaEmas",
            "hargaPlatinum",
            "hargaDiamond"][Math.floor(Math.random() * 4)]
        const random_buy = ["genap",
            "ganjil"][Math.floor(Math.random() * 2)]
...
// ==============
// fitur health dan energy
// ==============
function check_bot_rest(user_id){
    const [health, rest, rest_start] = dbSystem.getUserKeys(user_id, "role_play.health", "role_play.rest", "role_play.rest_start")
    console.log(c.red(`mengecek ${user_id}: ${health} health, ${rest} rest ${rest_start} start, lepas: ${Date.now() - rest_start >= 3*60*60*1000}`))
    if(health <= 10 && !rest){
        dbSystem.setUserKey(user_id, "role_play.rest", true)
        dbSystem.setUserKey(user_id, "role_play.rest_start", Date.now())
    } else if(rest && Date.now() - rest_start >= 3*60*60*1000 || health > 10){
        dbSystem.setUserKey(user_id, "role_play.rest", false)
        dbSystem.setUserKey(user_id, "role_play.health", 100)
    }
}

function buy_role_play(user_id, type){
    const [data_global, data_user] = [dbSystem.getGlobal(), dbSystem.getUser(user_id)]
    const toko = data_global.shop_on.filter((v) => v.type === type)
    if(toko.length === 0) return
    const toko_dipilih = toko[Math.floor(Math.random() * toko.length)]
    const detail_toko = dbSystem.getUserKey(toko_dipilih.owner_id, `bisnis.${toko_dipilih.bisnis_id}`)
    
    //proses pembelian
    let total_harga, jumlah_dibeli
    const dibutuhkan = 100 - data_user.role_play[type]
    jumlah_dibeli = dibutuhkan
    total_harga = jumlah_dibeli * detail_toko.bisnis_harga_jual
    if(total_harga > data_user.uang) {
        jumlah_dibeli = Math.floor(data_user.uang / detail_toko.bisnis_harga_jual)
        total_harga = jumlah_dibeli * detail_toko.bisnis_harga_jual
    }
    
    if(jumlah_dibeli === 0)return
    
    //tambah ke db user_id
    dbSystem.updateUser(user_id, {
        uang: "add -"+total_harga,
        role_play: {
            [type]: "add "+jumlah_dibeli
        }
    })
    
    // tambah ke db owner bisnis
    dbSystem.updateUser(detail_toko.bisnis_owner_id, {
        bisnis: {
            [detail_toko.bisnis_id]: {
                bisnis_stok: "add -"+jumlah_dibeli,
                bisnis_total_pembeli: "add 1",
                bisnis_barang_terjual: "add "+jumlah_dibeli,
                bisnis_uang_tersedia: "add "+total_harga
            }
        }
    })
}
