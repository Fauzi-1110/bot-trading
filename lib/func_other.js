...
function formatNumber(uang){
    return `${uang >= 1e21 ? (Number(uang) || 0) : (uang.toLocaleString("id") || 0)}`
}
...
function buat_bisnis(user_id){
    ...
    if(Math.floor(uang * 0.5) <= 5000000000000)return console.log(c.blueBright(`${user_id} ditolak membuat bisnis! [uang kurang]`))
    if(Object.keys(bisnis).length >= 3)return console.log(c.blueBright(`${user_id} ditolak membuat bisnis! [punya 3 bisnis]`))
...
function restock_bisnis(){
    ...
    for(let [id, data] of bot_punya_bisnis){
        ...
        for(let [bisnis_id, bisnis_data] of bot_bisnis_entries){
            ...
            //pengecekan bangkrut
            if(bisnis_data == null)continue
            if(bisnis_data.bisnis_stok <= -100000){
                temp.bisnis[bisnis_id] = null
                
                const shop_on = dbSystem.getGlobalKey("shop_on")
                const index = shop_on.findIndex(v => v.bisnis_id === bisnis_data.bisnis_id)
                shop_on.splice(index, 1)
                dbSystem.setGlobalKey("shop_on", shop_on)
                continue
            } 
            
            //sistem jumlah & harga pembelian
            if(bisnis_data.bisnis_stok >= 100000) dibeli = 500
            else if (bisnis_data.bisnis_stok <= 100) dibeli = 1000
