...
class dbSystem {
    ...
    static async createDBUser(type, data) {
        ...
        const merge_user = mergeWith(def, readDB[id], function(def_value, old_value, key, data_lama, data_baru){
            ...
            //default akhir
            if(old_value != undefined && old_value !== "delete" && def_value == undefined){
                console.log(c.red(`change value to \"delete\" in user {${id}} old_data: ${old_key}`))
                return "delete"
            } else if(old_value == undefined && typeof def_value == "string") return ""
              else if(old_value == undefined && typeof def_value == "number") return 0
        })
