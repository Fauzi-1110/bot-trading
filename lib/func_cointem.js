...
async function randCoin() {
    ...
    for(let coin_name of coin_list){
        let e_volume, e_chart, e_waktu, e_random, e_posisi
        ...
        if( Math.floor(Date.now() / 60000) % 2 === 0){
            ...
            volume_beli = global_data[coin_name+"_volume"].beli_ganjil
            volume_jual = global_data[coin_name+"_volume"].jual_ganjil
        }
        ...
        for(let data_pola of pola_list){
            ...
            if(data_pola.pola.every((v, i) => v === chart_pola[i])) {pola_take = data_pola; break;}
        }
        if(pola_take){
        ...
        //dari waktu
        const jam_now = Number(moment_tz.tz("Asia/Jakarta").format("HH"))
        if(jam_now > 20 || jam_now < 4) e_waktu = Math.floor(Math.random() * 8) - 4
        else e_waktu = 0
        
        //dari random
        e_random = Math.floor(Math.random() * 6) - 3
        
        //dari posisi
        const harga_sekarang = global_data[coin_name]
        if(harga_sekarang < min || Math.floor(harga_sekarang * 0.85) < min)e_posisi = Math.floor(Math.random() * 6) +2
        else if (harga_sekarang > max) e_posisi = -(Math.floor(Math.random() * 6) +2)
        else e_posisi = 0
        
        //inisialisasi buat besarnya naik
        const total_persen_int = e_volume + e_chart + e_waktu + e_random + e_posisi
        const total_persen = total_persen_int/100
        const jumlah_nambah = Math.floor(Math.abs(total_persen) * harga_sekarang ) * (total_persen_int < 0 ? -1 : 1)
        
        //update chart
        let posisi
        if(harga_sekarang+jumlah_nambah > harga_sekarang) posisi = "1"
        else posisi = "0"
        chart_pola.shift()
        chart_pola.push(posisi)
        dbSystem.setGlobalKey(coin_name.replace("Coin", "Chart"), chart_pola)
        
        // simpan harga ke db
        if(harga_sekarang + jumlah_nambah <= 50000) dbSystem.setGlobalKey(coin_name, 75000)
        else dbSystem.addGlobalKey(coin_name, jumlah_nambah)
        ...
        addHistoryPerubahanHarga({
            ...
            e_random: e_random,
            e_posisi: e_posisi,
            jumlah_efek: total_persen_int,
            jumlah_nambah: jumlah_nambah,
            harga_lama: harga_sekarang,
            harga_baru: harga_sekarang+jumlah_nambah
        }, coin_name)
    }
